...
| `library_view` | IPN | ipn, description, symbol, footprint, source, pinout_checked | What is drawn and what is not |
...
| `library_view` | `select name, ipn, description, symbol, footprint, source, pinout_checked from parts_table order by symbol is not null, footprint is not null, ipn;` |
